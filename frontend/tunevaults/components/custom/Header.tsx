    ...
    <header className={`bg-background/80 backdrop-blur-sm border-b sticky top-0 z-50 transition-all duration-300 ${isScrolled ? 'py-2' : 'py-4'}`}>
      <div className="container mx-auto px-4 flex justify-between items-center">
        <Link href="/" className="text-2xl font-bold text-primary flex items-center">
          <RotatingVinylDisc />
          <span className="bg-clip-text text-transparent bg-gradient-to-r from-primary to-primary/60">Songfer</span>
        </Link>
        <nav className="flex items-center space-x-2 md:space-x-4">
          ...
          <Link href="https://github.com/yourusername/Songfer" target="_blank" rel="noopener noreferrer">
            <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full">
