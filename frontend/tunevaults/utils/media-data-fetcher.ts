...
export const fetchSpotifyData = async (id: string, isPlaylist: boolean, url: string, authToken?: string): Promise<MediaPreviewData> => {
  try {
    ...
    const fetchUrl = `http://127.0.0.1:8000//api/songs/spotify/${endpoint}/${id}/`;
    ...
    if (!response || !response.ok) {
        ...
        const altUrl = `http://127.0.0.1:8000//api/songs/spotify-${isPlaylist ? 'playlist' : 'track'}/${id}/`;
        ...
        if (!altResponse || !altResponse.ok) {
            const thirdUrl = `http://127.0.0.1:8000//api/spotify/${endpoint}/${id}/`;
...
export const getPlaylistDownloadEndpoint = (playlistId: number | string): string => {
  return `http://127.0.0.1:8000//api/songs/playlists/${playlistId}/download-all/`;
};
